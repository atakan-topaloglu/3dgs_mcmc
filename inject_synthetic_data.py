...
def create_synthetic_camera(ref_camera: pycolmap.Camera, 
    ...
    """
    print(f"Creating synthetic camera with ID {new_id} based on reference camera ID {ref_camera.camera_id}")
...
def main():
    ...
    # 1. Create the synthetic camera
    ref_cam_id = min(rec.cameras.keys())
    ref_cam = rec.cameras[ref_cam_id]
    new_cam_id = (max(rec.cameras.keys()) + 1) if rec.cameras else 1
    synth_cam = create_synthetic_camera(ref_cam, new_cam_id, args.width, args.height)
    ...
    for image in original_images:
        ...
        # --- KEY CHANGE: Associate the new frame with the NEW synthetic rig ---
